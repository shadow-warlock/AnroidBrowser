...
public class DownloadAdapter extends BaseAdapter{
    ...
    @Override
    public int getCount() {
        return p.getDownloadsList().size();
    }

    @Override
    public Object getItem(int position) {
        return p.getDownloadsList().get(position);
    }
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        // и картинка
        ((TextView) view.findViewById(R.id.title)).setText(p.getDownloadsList().get(position).getTitle());
        ((TextView) view.findViewById(R.id.url)).setText(p.getDownloadsList().get(position).getUrl());
